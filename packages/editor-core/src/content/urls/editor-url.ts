export function sanitizeEditorLinkUrl(value: unknown): string | null {
  ...
  if (trimmed.startsWith("//"))
    return looksLikeAbsoluteHost(trimmed.slice(2)) ? `https:${trimmed}` : null;
  if (
    trimmed.startsWith("#") ||
    trimmed.startsWith("/") ||
    trimmed.startsWith("./") ||
  ...
  if (!protocolMatch)
    return looksLikeAbsoluteHost(trimmed) ? `https://${trimmed}` : null;
  if (!trimmed.includes("://") && looksLikeHostWithPortPath(trimmed))
...
function looksLikeHostWithPortPath(value: string): boolean {
  return /^(?:localhost|[a-zA-Z\d-]+(?:\.[a-zA-Z\d-]+)+):\d+(?:[/?#].*)?$/.test(
    value,
  );
}
