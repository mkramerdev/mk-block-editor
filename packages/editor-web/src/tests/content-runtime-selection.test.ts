...
describe("editor content runtime selection", () => {
  it("creates and destroys exactly one default runtime", () => {
    ...
    expect(editor.readBlockPlainText(blockId, "textBlock")).toBe("default");
  ...
  it("uses one definition-provided runtime without constructing the fallback", () => {
    ...
    expect(editor.readBlockPlainText(blockId, "textBlock")).toBe("custom");
    expect(editor.insertText({ blockId, offset: 6, text: " runtime" })).toBe(
      true,
    );
    expect(selected.readBlockPlainText(blockId, "textBlock")).toBe(
      "custom runtime",
    );
  ...
  it("uses and destroys one definition-provided runtime", () => {
    const sourceSnapshot = snapshot("custom runtime");
    const selected = createEditorContentRuntime(
      createEditorContentStartup(sourceSnapshot, testEditableEditorDefinition),
    );
    const destroy = vi.spyOn(selected, "destroy");
    ...
    expect(editor.readBlockContent(blockId, "textBlock")).toMatchObject({
      content: [{ content: [{ text: "custom runtime" }] }],
    });
...
function snapshot(text: string) {
  return createTestEditorSnapshot([{ id: blockId, type: "textBlock", text }]);
}
