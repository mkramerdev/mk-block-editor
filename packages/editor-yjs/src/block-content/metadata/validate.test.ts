...
describe("block content metadata validation", () => {
	it("detects metadata drift and assertion failures", () => {
		...
		context.metadata.set(EDITOR_YJS_BLOCK_CONTENT_METADATA_KEYS.blockId, BLOCK_B);
	...
	it("reports exact validation failures for context and metadata drift", () => {
		...
		expect(validateBlockContentDocContext({
			...invalidContextId,
			blockId: "" as BlockId,
		})).toMatchObject({
			ok: false,
			reason: "invalid-context-id",
			message: "block content context blockId must be a non-empty structural key",
		});
