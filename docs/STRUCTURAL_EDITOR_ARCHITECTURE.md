...
| -------------------------------------------------- | ---------------------------------------------------------------------- |
| Block action menu                                  | Insert before/after, duplicate, or delete through block operations     |
| Document block drag-and-drop                       | Product drop planning followed by one canonical move transaction       |
...
First Draft mounts delete, duplicate, and insertion actions in its block action
menu. Its drag handle shares that trigger and starts document drag-and-drop only
after pointer activation.

## Structural keyboard behavior

The neutral text host emits Enter, boundary Backspace, and forward boundary
Delete requests. First Draft owns the product planners that interpret its
paragraphs, lists, wrappers, tables, and restorative defaults, then submits one
generic structural transaction plan. Forward Delete retains the current text
block as the survivor when the product plan joins a following compatible block.
...
active binding; disposal destroys the shared view.

## Block drag-and-drop

First Draft installs one product-owned drag provider for document blocks and
table rows/columns. Stable semantic drop anchors are registered from mounted
structure; preview snapshots are captured at activation and remain separate
from canonical document state. Pointer movement, targeting, overlays, and
autoscroll create no editor transaction. A valid document drop revalidates the
source and destination and calls the product move planner once; stale,
disconnected, incompatible, or already-moved sources are rejected. Table row
and column drops use their table-specific mutation planners.

The grip button keeps native `draggable={false}` because the integration uses
the `@mk-drag-and-drop/react` pointer lifecycle rather than browser native drag
events. The same button opens the block action menu when activated as an
ordinary click or keyboard control.
...
| Additional selection manager                                                | Editable editor runtime                  | Updated by remote ingress and released with the editor                                               |
