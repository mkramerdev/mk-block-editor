...
The block-operation API moves and duplicates blocks. First Draft exposes those
operations through its block action menu and installs a product-owned document
drag-and-drop producer. Drag previews and target updates are transient; only a
validated drop invokes the move planner and crosses the canonical transaction
boundary.
...
finalized semantic editor transaction
  ...
  -> in-memory unresolved-transaction publisher
  -> WebSocket publication for the active connection generation
...
effects into the current wire transaction. The mounted surface retains
unresolved finalized transactions in an in-memory FIFO publisher, preserves
their identities across socket generations, and resends only after revision
catch-up makes the generation usable. The realtime service serializes
persistence for a document, accepts against the current PostgreSQL revision,
acknowledges the sender, and broadcasts an accepted replay to other subscribed
clients. Clients correlate their own accepted replays without applying them
twice and apply unseen peer replays through `applyRemoteTransaction`.
...
implemented. The in-memory publisher can survive a mounted surface's socket
retry, but it cannot survive page closure or process loss and does not permit
